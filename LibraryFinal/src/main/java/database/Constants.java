...
public class Constants {
    ...
    public static Map<String, List<String>> getRolesRights() {
        Map<String, List<String>> roleRights = new HashMap<>();
        for (String role : Roles.ROLES) {
            roleRights.put(role, new ArrayList<>());
        }

        roleRights.get(Roles.ADMINISTRATOR).addAll(Arrays.asList(RIGHTS));

        roleRights.get(EMPLOYEE).addAll(Arrays.asList(CREATE_BOOK, DELETE_BOOK, UPDATE_BOOK, SELL_BOOK));

        roleRights.get(Roles.CUSTOMER).addAll(Arrays.asList(SELL_BOOK, BUY_BOOK, RETURN_BOOK));
    ...
    public static class Tables {
        ...
        public static final String[] ORDERED_TABLES_FOR_CREATION =
                new String[]{USER, ROLE, RIGHT, ROLE_RIGHT, USER_ROLE, BOOK, SALE};
    }
